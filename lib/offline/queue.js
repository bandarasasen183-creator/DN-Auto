 ...
 *             recording that a bill was paid and signed for.
 *
 *   Never   — anything that actually moves money. This app never touches
 *             the card machine — WEBXPAY has no API — so "complete" on
 *             the handover screen only *records* that a mechanic took a
 *             payment and the customer signed for it. It doesn't charge
 *             anyone. That is what makes it safe to queue: a duplicate
 *             record is a bookkeeping error, not a duplicate charge, and
 *             it is guarded against below anyway.
 *
 *             What genuinely cannot queue is anything the server itself
 *             must be the one to decide — a refund (has to check what is
 *             left to refund against the real, current total) and a
 *             team-created promo code (has to check the live usage
 *             count). Those still need a connection.
 *
