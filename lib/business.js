...
export const BUSINESS = {
  ...
  contact: {
    ...
    email: 'hello@dnauto.org',       // TODO: confirm the real mailbox
  },
};

/** Maps a service slug to an icon in components/Icon.js. */
export const SERVICE_ICONS = {
  'general-service': 'wrench',
  'oil-filter-change': 'car',
  'brake-check': 'shield',
  'engine-diagnostics': 'chart',
  'electrical-battery': 'alert',
  'transmission-service': 'settings',
  'pre-purchase-inspection': 'check',
};
...
/**
 * Days that can be booked online, as JS day numbers.
 *
 * Only Sunday. Weekday evenings are emergency work for existing customers,
 * and an emergency is arranged by phone with the workshop — never dropped
 * into the diary by a form, because someone has to confirm a mechanic is
 * actually free before the customer drives over.
 */
export const ONLINE_BOOKING_DAYS = [0];

export function isBookableOnline(date) {
  const when = date instanceof Date ? date : new Date(date);
  if (Number.isNaN(when.getTime())) return false;
  return ONLINE_BOOKING_DAYS.includes(when.getDay());
}

/**
 * Validates a requested slot. Online booking is Sunday only; everything else
 * is refused here and pointed at the phone.
 */
export function validateSlot(date) {
  const when = date instanceof Date ? date : new Date(date);
  ...
  // Emergency slots exist, but they are not self-service.
  if (!ONLINE_BOOKING_DAYS.includes(when.getDay())) {
    return {
      ok: false,
      reason:
        'Online booking is for Sunday appointments. Weekday evenings are emergency ' +
        'repairs for existing customers and are arranged by phone — please call the ' +
        'workshop and we will fit you in.',
      callInstead: true,
    };
  }
  ...
  return { ok: true, isEmergency: false };
}
